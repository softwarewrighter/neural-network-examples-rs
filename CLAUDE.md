...
1. **READ `documentation/process.md` COMPLETELY** - This document defines the REQUIRED development process and is MANDATORY
   - Technology stack constraints (Rust/Yew/WASM only - NO JavaScript/TypeScript/Python)
   - TDD workflow (Red-Green-Refactor)
   - Code quality standards (zero warnings policy)
   - UI verification requirements (MCP/Playwright)
   - Pre-commit quality checks (fmt, build, clippy, test)
   - Build/serve script requirements
   - **VIOLATIONS OF THIS PROCESS ARE NOT ACCEPTABLE**
2. **READ `documentation/learnings.md` COMPLETELY** - Review all lessons learned from previous phases
...
3. Check for specific warnings about past mistakes to avoid
4. Note any project-specific patterns or decisions
5. Verify understanding of test requirements for current task

**AFTER completing ANY incremental change:**
1. **UPDATE `documentation/learnings.md`** in the relevant section
...
**Purpose:** This ensures continuous learning and prevents repeatedly making the same mistakes across sessions. Treat `documentation/learnings.md` as the project's institutional memory and `documentation/process.md` as the project's non-negotiable development contract.
...
See `documentation/learnings.md` "Testing Strategy" section for complete requirements and patterns.
...
- **Documentation:** `documentation/` for architecture/planning/process, rustdoc for API
- **Build artifacts:** `docs/` will be used for GitHub Pages (static site build output)
...
See `documentation/plan.md` for detailed phase breakdown.

## Documentation References

**Before implementing features, read:**
- **`documentation/process.md`** - ⚠️ READ FIRST - MANDATORY development process (TDD, quality checks, technology constraints)
- **`documentation/learnings.md`** - ⚠️ READ SECOND - Past mistakes, lessons learned, patterns to follow/avoid
- `documentation/architecture.md` - Technical architecture, design decisions, algorithms
- `documentation/plan.md` - 6-phase implementation plan with code snippets and C++ mappings
- `documentation/PRD.md` - Requirements, success metrics, future roadmap

**For specific topics:**
- Error types: `src/error.rs`
- Activation functions: `src/activation.rs` (trait pattern reference)
- Network structure: `documentation/architecture.md` sections 1-2
- Algorithms: `documentation/architecture.md` "Algorithm Details"
