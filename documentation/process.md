...
3. **ASCII-Only Markdown**: All markdown files MUST be ASCII-only
   - NO Unicode characters (smart quotes, em dashes, special symbols, etc.)
   - Use plain ASCII equivalents: `"` instead of `"` or `"`, `-` instead of `—`
   - Use `->` instead of `→`, `...` instead of `…`
   - Prevents encoding issues across platforms and tools
   - **Enforced by automated test** in `tests/markdown_ascii_test.rs`

4. **Documented**: All public items MUST have:
   ```rust
...
5. **Tested**: Every public function/module/crate MUST have tests
...
### 5. Markdown ASCII Check
```bash
cargo test --test markdown_ascii_test
```
**Requirement**: All markdown files must be ASCII-only (no Unicode characters)

**DO NOT**:
- Use Unicode characters in markdown files (smart quotes, em dashes, arrows, ellipses, etc.)
- Copy/paste text with Unicode formatting from other sources
- Use special symbols or characters

**DO**:
- Use plain ASCII equivalents (`"` not `"`, `-` not `—`, `->` not `→`, `...` not `…`)
- Verify with the automated test before committing
- Fix any Unicode characters found
...
6. Pre-commit checks:
   ...
   - cargo test --test markdown_ascii_test (markdown files are ASCII-only)
...
- ✅ Use ASCII-only markdown files (no Unicode characters)
- ✅ Have updated screenshots if UI changed
- ✅ Pass all quality checks (fmt, build, clippy, test, markdown ASCII)
